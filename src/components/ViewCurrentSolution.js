...
import fetchCurrentSoln from '../lib/fetchCurrentSoln';
import tableInfo from '../lib/tableToJson';

//Defining react component to be rendered
class ViewCurrentSolution extends React.Component {
  ...
  // Add the async function (e.g. fetchCurrentSoln) to fetch data to be displayed in table
  componentDidMount () {
    debugger;
    let {store, viya} = this.context;
    console.log(this.context)
    debugger;
    fetchCurrentSoln(viya.appEnv, viya.session, store)
    .then(r =>{
      this.setState({
        data: r
      })
  ...
  // Finally render the table - change the fieldnames, column headers and add various formats in lines 160-164 or add more
  render() {
    debugger;
...
export default ViewCurrentSolution;
